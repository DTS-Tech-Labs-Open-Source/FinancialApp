//
//  Constants.swift
//  FinancialApp
//
//  Created by Dilan Tharidu Sangeeth on 3/9/20.
//  Copyright © 2020 Dilan Tharidu Sangeeth. All rights reserved.
//

public class Constants{
    
    static let TAG_NUMBER_1 = 1
    static let TAG_NUMBER_2 = 2
    static let TAG_NUMBER_3 = 3
    static let TAG_NUMBER_4 = 4
    static let TAG_NUMBER_5 = 5
    static let TAG_NUMBER_6 = 6
    static let TAG_NUMBER_7 = 7
    
    
    static let INTEREST_UNIT_TIME = 12
    static let COMPOUND_USER_DEFAULTS_KEY = "compound"
    static let SAVEING_USER_DEFAULTS_KEY = "saving"
    static let LOAN_USER_DEFAULTS_KEY = "loan"
    static let MORTGAGE_USER_DEFAULTS_KEY = "mortgage"
    static let EXCHANGE_RATE_USER_DEFAULTS_KEY = "exchange"
    
    static let USER_SAVE_MAX_DEFAULTS_COUNT = 5
    
}
